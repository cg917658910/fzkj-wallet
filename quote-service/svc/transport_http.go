...
func MakeHTTPHandler(endpoints Endpoints, responseEncoder httptransport.EncodeResponseFunc, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/v1/quote/fiat").Handler(httptransport.NewServer(
		endpoints.FiatQuoteEndpoint,
		DecodeHTTPFiatQuoteZeroRequest,
		responseEncoder,
		serverOptions...,
	))
	return m
...
// DecodeHTTPFiatQuoteZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded fiatquote request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPFiatQuoteZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.FiatQuoteRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
	...
	if SymbolFiatQuoteStrArr, ok := queryParams["symbol"]; ok {
		SymbolFiatQuoteStr := SymbolFiatQuoteStrArr[0]
		SymbolFiatQuote := SymbolFiatQuoteStr
		req.Symbol = SymbolFiatQuote
	}

	if FiatFiatQuoteStrArr, ok := queryParams["fiat"]; ok {
		FiatFiatQuoteStr := FiatFiatQuoteStrArr[0]
		FiatFiatQuote := FiatFiatQuoteStr
		req.Fiat = FiatFiatQuote
	}
