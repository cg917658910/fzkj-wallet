...
type Endpoints struct {
	...
	FiatQuoteEndpoint   endpoint.Endpoint
}
...
func (e Endpoints) FiatQuote(ctx context.Context, in *pb.FiatQuoteRequest) (*pb.FiatQuoteResponse, error) {
	response, err := e.FiatQuoteEndpoint(ctx, in)
	if err != nil {
		return nil, err
	}
	return response.(*pb.FiatQuoteResponse), nil
}

// Make Endpoints

func MakeHealthCheckEndpoint(s pb.QuoteServiceServer) endpoint.Endpoint {
...
func MakeFiatQuoteEndpoint(s pb.QuoteServiceServer) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (response interface{}, err error) {
		req := request.(*pb.FiatQuoteRequest)
		v, err := s.FiatQuote(ctx, req)
		if err != nil {
			return nil, err
		}
		return v, nil
...
func (e *Endpoints) WrapAllExcept(middleware endpoint.Middleware, excluded ...string) {
	...
	for inc := range included {
		...
		if inc == "FiatQuote" {
			e.FiatQuoteEndpoint = middleware(e.FiatQuoteEndpoint)
		}
	}
}
...
func (e *Endpoints) WrapAllLabeledExcept(middleware func(string, endpoint.Endpoint) endpoint.Endpoint, excluded ...string) {
	...
	for inc := range included {
		...
		if inc == "FiatQuote" {
			e.FiatQuoteEndpoint = middleware("FiatQuote", e.FiatQuoteEndpoint)
		}
	}
}
