...
func MakeHTTPHandler(endpoints Endpoints, responseEncoder httptransport.EncodeResponseFunc, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/order/notify/start").Handler(httptransport.NewServer(
		endpoints.OrderNotifyStartEndpoint,
		DecodeHTTPOrderNotifyStartZeroRequest,
		responseEncoder,
		serverOptions...,
	))

	m.Methods("GET").Path("/order/notify/stop").Handler(httptransport.NewServer(
		endpoints.OrderNotifyStopEndpoint,
		DecodeHTTPOrderNotifyStopZeroRequest,
		responseEncoder,
		serverOptions...,
	))
	return m
...
// DecodeHTTPOrderNotifyStartZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded ordernotifystart request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPOrderNotifyStartZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.OrderNotifyStartRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
...
// DecodeHTTPOrderNotifyStopZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded ordernotifystop request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPOrderNotifyStopZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.OrderNotifyStopRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
