...
func MakeHTTPHandler(endpoints Endpoints, responseEncoder httptransport.EncodeResponseFunc, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("GET").Path("/v1/wash/shortmsg").Handler(httptransport.NewServer(
		endpoints.WashShortMsgEndpoint,
		DecodeHTTPWashShortMsgZeroRequest,
		responseEncoder,
		serverOptions...,
	))
	return m
...
// DecodeHTTPWashShortMsgZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded washshortmsg request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPWashShortMsgZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.WashShortMsgRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
	...
	if MsgWashShortMsgStrArr, ok := queryParams["msg"]; ok {
		MsgWashShortMsgStr := MsgWashShortMsgStrArr[0]
		MsgWashShortMsg := MsgWashShortMsgStr
		req.Msg = MsgWashShortMsg
	}

	if MsgIdWashShortMsgStrArr, ok := queryParams["msg_id"]; ok {
		MsgIdWashShortMsgStr := MsgIdWashShortMsgStrArr[0]
		MsgIdWashShortMsg := MsgIdWashShortMsgStr
		req.MsgId = MsgIdWashShortMsg
	}

	if CurrencyWashShortMsgStrArr, ok := queryParams["currency"]; ok {
		CurrencyWashShortMsgStr := CurrencyWashShortMsgStrArr[0]
		CurrencyWashShortMsg := CurrencyWashShortMsgStr
		req.Currency = CurrencyWashShortMsg
	}

	if TypeNameWashShortMsgStrArr, ok := queryParams["type_name"]; ok {
		TypeNameWashShortMsgStr := TypeNameWashShortMsgStrArr[0]
		TypeNameWashShortMsg := TypeNameWashShortMsgStr
		req.TypeName = TypeNameWashShortMsg
	}
