...
func NewEndpoints(service pb.WashServiceServer) svc.Endpoints {
	// Business domain.
	...
	var (
		...
		washshortmsgEndpoint = svc.MakeWashShortMsgEndpoint(service)
	)
...
func Run(cfg svc.Config) {
	...
	go func() {
		...
		s := grpc.NewServer()
		pb.RegisterWashServiceServer(s, srv)
